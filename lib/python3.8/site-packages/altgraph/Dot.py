...
class Dot(object):
    ...
    def display(self, mode="dot"):
        ...
        if mode == "neato":
            ...
            subprocess.run(neato_cmd, shell=False, check=True, text=True)
        else:
            self.save_dot(self.temp_dot)

        plot_cmd = "%s %s" % (self.dotty, self.temp_dot)
        subprocess.run(plot_cmd, shell=False, check=True, text=True)
    ...
    def save_img(self, file_name=None, file_type="gif", mode="dot"):
        ...
        subprocess.run(create_cmd, shell=False, check=True, text=True)
